...
async function getCroppedBlob(
    ...
    const type = ALLOWED_MIME.has(mime) ? mime : "image/jpeg";
    return new Promise((resolve) => canvas.toBlob((b) => resolve(b as Blob), type, 0.92));
}
...
export default function BoardDetailsModal({ open, onClose, orgId, board, onSaved }: Props) {
    ...
    /* Seed form when full row arrives */
    useEffect(() => {
        if (!open) return;
        const src = full ?? board;
        if (!src) return;
        const numToStr = (n: number | null | undefined) => (typeof n === "number" && Number.isFinite(n) ? String(n) : "");
        setForm({
            ...
            width_ft: numToStr(src.width_ft) || (parseFeetFromDisplay(src.width_display) != null ? String(parseFeetFromDisplay(src.width_display)) : ""),
            height_ft: numToStr(src.height_ft) || (parseFeetFromDisplay(src.height_display) != null ? String(parseFeetFromDisplay(src.height_display)) : ""),
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const BASELINE_TYPES = ["Bulletin", "Poster", "Digital", "Static", "Wallscape", "Transit", "Mobile"];
            const groupNames = new Set<string>();
            const types = new Set<string>(BASELINE_TYPES);

            const sg = await supabase.from("spec_groups").select("name, board_type").eq("organization_id", orgId).order("name", { ascending: true });
            if (!sg.error && sg.data) {
            ...
            const bd = await supabase.from("boards").select("spec_group, board_type").eq("organization_id", orgId);
            if (!bd.error && bd.data) {
    ...
    /* Derived displays */
    const widthDisplay = useMemo(() => feetDisplay(numOrNull(form.width_ft)) ?? (current?.width_display ?? "—"), [form.width_ft, current?.width_display]);
    const heightDisplay = useMemo(() => feetDisplay(numOrNull(form.height_ft)) ?? (current?.height_display ?? "—"), [form.height_ft, current?.height_display]);
    const latitudeDisplay = useMemo(() => latDisp(numOrNull(form.latitude)) ?? (current?.latitude_display ?? "—"), [form.latitude, current?.latitude_display]);
    const longitudeDisplay = useMemo(() => lngDisp(numOrNull(form.longitude)) ?? (current?.longitude_display ?? "—"), [form.longitude, current?.longitude_display]);
    ...
    const usageBarColor = usagePct < 70 ? "bg-green-500" : usagePct < 90 ? "bg-amber-500" : "bg-red-500";
    ...
    const doUpload = async () => {
        ...
        try {
            ...
            const { error: upErr } = await supabase.storage.from("board-photos").upload(newPath, file!, {
            ...
            toast.error(`Image update failed: ${e?.message || "Unknown error"}`);
        } finally {
            setUploading(false);
        }
    };
    ...
    return (
        <>
            ...
            />
            <div className={`fixed inset-0 z-[91] flex items-center justify-center p-4 ${open ? "" : "pointer-events-none opacity-0"}`}>
                            ...
                            <div className="space-y-6">
                                ...
                                <section>
                                    ...
                                    <div className="relative">
                                        <div
                                            className={
                                                "relative rounded-lg bg-zinc-50 dark:bg-zinc-900 h-64 flex items-center justify-center text-sm text-zinc-500 " +
                                                "border border-dashed border-zinc-300 dark:border-zinc-700 overflow-hidden"
                                            }
                                            onDragOver={(e) => e.preventDefault()}
                                            onDrop={(e) => { e.preventDefault(); onPickFile(e.dataTransfer.files); }}
                                            onClick={() => !cropMode && document.getElementById("edit-hero-input")?.click()}
                                        >
                                                    ...
                                                    <>
                                                        ...
                                                        <button
                                                            onClick={async () => {
                                                                ...
                                                                const blob = await getCroppedBlob(previewUrl, croppedAreaPixels, file.type);
                                                                const ext = (file.name.split(".").pop() || "jpg").toLowerCase();
                                                                const croppedFile = new File(
                                                                    [blob],
                                                                    file.name.replace(/\.(png|jpg|jpeg)$/i, "") + "-cropped." + ext,
                                                                    { type: blob.type }
                                                                );
                                                                if (previewUrl) URL.revokeObjectURL(previewUrl);
                                                            ...
                                                            className="px-3 py-1.5 rounded-full border border-blue-600 bg-blue-600 text-white text-sm"
                                                        >
                                                            Apply crop
                                                        </button>
                                                    </>
